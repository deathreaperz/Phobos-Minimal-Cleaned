...
{
	return (this->GetRange(pData).WidthOrRange > 0) ? SuperWeaponFlags::NoEvent : SuperWeaponFlags::None;
}
...
bool SW_SonarPulse::Activate(SuperClass* pThis, const CellStruct& Coords, bool IsPlayer)
{
	...
	auto Detect = [pThis, pData](TechnoClass* pTechno) -> bool
		{
			...
			nTime.Start(MaxImpl(nTime.GetTimeLeft(), pData->Sonar_Delay.Get()));
