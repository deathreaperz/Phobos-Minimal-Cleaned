...
bool SW_HunterSeeker::Activate(SuperClass* pThis, const CellStruct& Coords, bool IsPlayer)
{
	...
	if (!pType)
	{
		Debug::Log("HunterSeeker super weapon \"%s\" could not be launched. No HunterSeeker unit type set for house \"%ls\".\n", pThis->Type->ID, pOwner->UIName);
		return false;
	}

	const auto Count = (pExt->SW_MaxCount >= 0) ? static_cast<size_t>(pExt->SW_MaxCount) : std::numeric_limits<size_t>::max();

	size_t Success = 0;
	Helpers::Alex::for_each_if_n(pOwner->Buildings.begin(), pOwner->Buildings.end(), Count,
		...
		[=, &Success](BuildingClass* pBld)
 {
	...
	if (!Success)
	{
		Debug::Log("HunterSeeker super weapon \"%s\" could not be launched. House \"%ls\" does not own any HSBuilding or No Buildings attached with this HSType Superweapon.\n", pThis->Type->ID, pOwner->UIName);
	}
...
{
	const auto HSBuilding = !pData->HunterSeeker_Buildings.empty() ? make_iterator(pData->HunterSeeker_Buildings) : make_iterator(RulesExtData::Instance()->HunterSeekerBuildings);
